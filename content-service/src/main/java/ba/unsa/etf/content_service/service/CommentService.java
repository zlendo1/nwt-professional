...
public class CommentService {
  ...
  public List<CommentDto> getCommentsByPost(Post post) {
    return commentRepository.findByPost(post).stream().map(CommentMapper::toDto).toList();
  }

  @Transactional(readOnly = true)
  public List<CommentDto> getCommentsByUser(User user) {
    return commentRepository.findByUser(user).stream().map(CommentMapper::toDto).toList();
  }

  @Transactional(readOnly = true)
  public List<CommentDto> getCommentsByPostdate(LocalDate postdate) {
    return commentRepository.findByPostdate(postdate).stream().map(CommentMapper::toDto).toList();
  }

  @Transactional(readOnly = true)
  public List<CommentDto> getCommentsByTextContaining(String text) {
    return commentRepository.findByTextContaining(text).stream().map(CommentMapper::toDto).toList();
  }
  ...
  public CommentDto createComment(CommentDto commentDto) {
    ...
    Post post =
        postRepository
            ...
            .orElseThrow(
                () -> new RuntimeException("Post not found with ID: " + commentDto.getPostId()));
    User user =
        userRepository
