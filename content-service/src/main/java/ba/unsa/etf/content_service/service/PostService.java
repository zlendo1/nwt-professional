...
public class PostService {
  ...
  public List<PostDto> getAllPosts() {
    ...
    return postMapper.toDto(
        posts); // PostMapper.toDto će obogatiti svaki PostDto s UserDto iz user-management-service
  }
  ...
  public Optional<PostDto> getPostById(Long id) {
    ...
    return postRepository
        .findById(id)
        .map(postMapper::toDto); // postMapper::toDto obogaćuje s UserDto
  }
  ...
  public PostDto createPost(PostDto postDto) {
    ...
    if (postDto.getUser() != null && postDto.getUser().getId() != null) {
      ...
      logger.info(
          "ID korisnika iz dolaznog PostDto (postDto.getUser().getId()): {}", userIdFromDto);
    } else {
      logger.error(
          "ID korisnika nije pronađen u PostDto (postDto.getUser() ili postDto.getUser().getId() je null).");
      throw new IllegalArgumentException(
          "Korisnički ID mora biti priložen unutar 'user' objekta u PostDto.");
    }
    ...
    User authorEntity =
        userRepository
            .findById(finalUserIdToFetch)
            .orElseGet(
                () -> {
                  logger.info(
                      "Lokalni User entitet (referenca) za ID: {} ne postoji. Kreiram novu referencu.",
                      finalUserIdToFetch);
                  User newUserReference =
                      new User(finalUserIdToFetch); // Koristi konstruktor s ID-om
                  return userRepository.save(
                      newUserReference); // Spremi novu referencu u lokalnu bazu
                });

    // Logiramo samo ID jer naš lokalni User entitet (authorEntity) sada ima samo ID
    logger.info(
        "Osigurana lokalna User referenca (entitet) u content-service bazi: ID={}",
        authorEntity.getId());
