...
public class FileController {
    ...
    public ResponseEntity<List<FileDto>> getAllFiles() {
        ...
        List<FileDto> fileDtos = fileMapper.toDto(files);  // Mapiranje entiteta u DTO (instanciranje mappera)
    ...
    public ResponseEntity<FileDto> getFileById(@PathVariable("id") Long id) {
        ...
        return file.map(value -> new ResponseEntity<>(fileMapper.toDto(value), HttpStatus.OK))  // Instanciranje mappera
                .orElseGet(() -> new ResponseEntity<>(HttpStatus.NOT_FOUND)); // Vraćanje 404 ako fajl ne postoji
    }
    ...
    public ResponseEntity<FileDto> createFile(@RequestBody FileDto fileDto) {
        ...
        return new ResponseEntity<>(createdFileDto, HttpStatus.CREATED);  // Vraćanje kreiranog fajla u DTO formatu
    }
    ...
    public ResponseEntity<Void> deleteFile(@PathVariable("id") Long id) {
        ...
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);  // Vraćanje odgovora bez sadržaja (no content)
    }
}
